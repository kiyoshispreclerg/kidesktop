                                 ...
                                 int *out_depth, int *out_has_submenu, int max_items, int *n)
{
     ...
     * An item can declare children-display=submenu and still land here
     * with zero actual children -- servers that populate a submenu lazily
     * (Electron's, at least) only do it in response to that specific
     * item's own AboutToShow, which a single recursive GetLayout never
     * sends; dbusmenu_fetch() checks out_has_submenu after this returns
     * and resolves any such item with its own AboutToShow+GetLayout round
     * trip. */
...
static int dbusmenu_fetch_one(const char *busname, const char *path, int32_t parent_id, int32_t depth,
                               MenuItem *out_items, int *out_ids, int *out_depth, int *out_has_submenu, int max_items)
{
                    ...
                    int *out_ids, int *out_depth, int max_items)
{
    ...
    if (n <= 0 || depth >= 0) {
        /* depth >= 0 asked for a bounded number of levels on purpose
         * (globalmenu.c's top-label fetch uses depth=1); an item stopping
         * short of its own children there is the requested shape, not a
         * lazy server -- only an unbounded (depth<0) fetch can tell the
         * two apart. */
        return n;
    }
    ...
    for (int pass = 0; pass < max_items; pass++) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            MenuItem sub_items[DBUSMENU_EXPAND_MAX];
