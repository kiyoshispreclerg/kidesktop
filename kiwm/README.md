...
| `new_window_output` | `pointer` | Which output a new window opens on when it did not ask for a position (most windows): `pointer` (whichever output the mouse is on when the window appears, the default), `primary`, or `largest` (biggest output by area). Dialogs and other transients always open on their parent's output; a window that asks to come up fullscreen fills the output it placed itself on. |
