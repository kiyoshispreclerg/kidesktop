...
| `resize_grip` | `12` | Width (pixels) of the invisible resize grip along a window's left, right and bottom edges: a plain click within this far of an edge resizes -- from the corner when two edges are in range, along one axis otherwise -- instead of going to the application, and hovering it shows the matching resize cursor. Works with or without a visible border. Not offered on a maximized window (it fills its output; there is nothing to drag its edges towards), but half-tiled windows keep it, so the shared edge of a tiled pair can be dragged without a modifier. Note that every pixel of it is a pixel the application doesn't get, so a wide grip can shadow a scrollbar sitting right at the window's edge; lower it if that gets in the way. `0` disables it (resizing then needs the modifier drag). |
| `resize_grip_top` | `4` | The same for the **top** edge of a *decorated* window. kiwm's frame has no top border -- the titlebar is its first row -- so this strip is taken off the titlebar's own top rows and deliberately wins over the button underneath it and over titlebar-drag-to-move, which is the only way to reach the top edge at all. Small on purpose for exactly that reason, and capped at half the titlebar height whatever you write here. `0` disables the top grip. An undecorated window has no such conflict and ignores this, using `resize_grip=` on all four edges. The rest of the titlebar (its whole body, buttons and ends included) is grip-free, so no resize cursor appears where there is no resize. |
...
- **Click-drag a window's edge or corner**: resize, no modifier needed, decoration or not -- see
  `resize_grip=`/`resize_grip_top=` above. A corner resizes both axes; an edge only its own. On a
  decorated window the top edge is a thin strip of the titlebar's top rows (4px by default), since
  the frame has no top border. Not offered on maximized
