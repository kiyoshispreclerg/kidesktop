 ...
 * max_width_px if positive. Caller owns the returned layout
 * (g_object_unref() it). */
 ...
 * at `x`, vertically centered within [top_y, top_y+box_h). Reports the
 * rendered pixel width via *out_w (NULL if the caller doesn't need it). */
