...
static void write_default_config(const char *path)
{
    ...
    fprintf(f,
        ...
        "# is under it -- from a corner where two edges meet, along one axis\n"
        "# on an edge. The ring sits entirely outside the window, so it\n"
        "# costs the application nothing and all four edges are the same\n"
        "# thickness. Works with or without a visible border. 0 disables it\n"
        "# (resizing then needs the modifier drag).\n"
        "resize_grip=12\n"
        "\n"
...
void config_load(void)
{
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        *eq = '\0';
        ...
        } else if (strcmp(key, "osd_other_desktops") == 0) {
            wm.osd_other_desktops = atoi(val) != 0;
