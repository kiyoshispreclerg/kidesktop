...
### What is Xnotify?
Xnotify is a small X extension present in [XiS, a XLibre fork](https://github.com/kiyoshispreclerg/xserver).
...
It works with static rules or dynamically with the external guardian (XisGuard).
[More about Xnotify here](https://github.com/kiyoshispreclerg/xserver/blob/master/doc/Xnotify.md).
...
`gcc -O2 -Wall -Wextra -pthread -D_FORTIFY_SOURCE=2 -fstack-protector-strong -o xisguard xisguard.c -lX11`
...
The `--allow ACTION` and `--deny ACTION` flags make XisGuard respond immediately to any program requesting that action,
...
```
xisguard --allow SCREEN --deny RECORD
```
...
```
xisguard --allow INPUT --allow SELECTION --deny RECORD --deny SCREEN
```
...
by creating the file ~/.config/xisguard/perms.conf with more rules, or just by allowing them when xisguard asks for permission.
...
XisGuard saves the runtime flags (`--no-pause`, `--quiet`, `--always-kill`, `--log-level`) to
`~/.config/xisguard/xnotify.conf` on every startup. If the program is launched with no flags the
